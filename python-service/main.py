...
@app.get("/documents/inventory")
async def get_document_inventory(namespace: str = ""):
    """Get inventory of documents in the knowledge base"""
    try:
        if not pinecone_service:
            raise HTTPException(status_code=503, detail="Pinecone service not initialized")
        
        inventory = await pinecone_service.get_document_inventory(namespace=namespace)
        
        return inventory
        
    except Exception as e:
        logger.error(f"Document inventory error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
